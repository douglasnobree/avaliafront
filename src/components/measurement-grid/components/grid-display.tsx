    ...
    <div className="max-w-full overflow-x-auto">
      <div className="inline-block min-w-min">
        ...
        <div className="flex gap-1 mb-1">
          <div className="w-12 text-xs font-semibold text-center"></div>
          {Array.from({ length: numEmissores }).map((_, i) => (
            <div key={i} className="w-16 text-xs font-semibold text-center">
        ...
        {Array.from({ length: numLinhas }).map((_, linhaIndex) => {
          ...
          return (
            <div key={linha} className="flex gap-1 mb-1">
              {/* Label da linha */}
              <div className="w-12 flex items-center justify-center text-xs font-semibold">
              ...
              {Array.from({ length: numEmissores }).map((_, emissorIndex) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      'w-16 h-16 rounded-full border-2 transition-all duration-150 hover:scale-105',
                      isMedido
